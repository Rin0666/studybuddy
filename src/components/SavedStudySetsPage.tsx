...
export default function SavedStudySetsPage() {
  ...
  const { list, listStatus, listError, refresh, remove, removeStatus, removeError } = useSavedStudySets();
        ...
        {list.length === 0 ? (
          <div className="rounded-2xl border border-border bg-white p-10 text-center">
            ...
            <p className="text-sm text-foreground/70 mb-6 max-w-sm mx-auto">
              When you save a study set, it will show up here so you can review or share it anytime.
            </p>
            <Link
              to="/"
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-on-primary transition-all hover:bg-secondary active:scale-[0.97] cursor-pointer"
              ...
              >
                <div>
                  <div className="flex items-start justify-between gap-3 mb-3">
                    ...
                    <button
                      onClick={async () => {
                        if (confirm("Delete this saved lesson?")) {
                ...
                <div className="mt-5 pt-4 border-t border-border flex items-center gap-2">
                  <button
                    onClick={() => navigate("/", { state: { studySet: item.payload } })}
